...
//! Integration tests for rusticate-review-trait-bound-mismatches
...
#[test]
#[serial]
fn test_review_trait_bound_mismatches_on_apas() -> Result<()> {
    let _ctx = TestContext::ensure_apas_at_script_commit("rust/src/review_trait_bound_mismatches.py")?;
    
    let binary_path = std::env::current_dir()?.join("target/release/rusticate-review-trait-bound-mismatches");
    ...
    // Format: "Summary: X files checked, Y potential issues"
    let mut files_checked = 0;
    let mut violations = 0;
    
    for line in stdout.lines() {
        if line.starts_with("Summary:") {
            // Extract numbers from "Summary: X files checked, Y potential issues"
