...
//! Integration tests for rusticate-review-single-trait-impl
...
#[test]
#[serial]
fn test_review_single_trait_impl_on_apas() -> Result<()> {
    let _ctx = TestContext::ensure_apas_at_script_commit("rust/src/review_single_trait_impl.py")?;
    
    let binary_path = std::env::current_dir()?.join("target/release/rusticate-review-single-trait-impl");
