...
//! Integration tests for rusticate-review-impl-order
...
#[test]
#[serial]
fn test_review_impl_order_on_apas() -> Result<()> {
    let _ctx = TestContext::ensure_apas_at_script_commit("rust/src/review_impl_order.py")?;
    
    let binary_path = std::env::current_dir()?.join("target/release/rusticate-review-impl-order");
