...
//! Integration tests for rusticate-review-where-clause-simplification
...
#[test]
#[serial]
fn test_review_where_clause_simplification_on_apas() -> Result<()> {
    let _ctx = TestContext::ensure_apas_at_script_commit("rust/src/review_where_clause_simplification.py")?;
    
    let binary_path = std::env::current_dir()?.join("target/release/rusticate-review-where-clause-simplification");
