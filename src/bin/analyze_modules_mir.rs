//! rusticate-analyze-modules-mir - Analyze Rust stdlib usage from MIR files
//!
//! This tool analyzes MIR (Mid-level Intermediate Representation) files to understand
//! how Rust codebases use the standard library. MIR provides fully-qualified paths
//! for all function calls, method invocations, and type usage.
//!
//! Usage:
//!   rusticate-analyze-modules-mir <path>     Analyze MIR files in <path>
...
impl Args {
    fn parse() -> Result<Self> {
        ...
        while let Some(arg) = args_iter.next() {
            match arg.as_str() {
                ...
                arg if !arg.starts_with('-') => {
                    mir_path = Some(PathBuf::from(arg));
                }
                _ => {
        ...
        let mir_path = mir_path.context("Must specify path to MIR files\nRun with --help for usage")?;
        
        if !mir_path.exists() {
            bail!("Path does not exist: {}", mir_path.display());
        }
        if !mir_path.is_dir() {
            bail!("Path is not a directory: {}", mir_path.display());
        }

        Ok(Args { mir_path, jobs })
    }
}

fn print_help() {
    println!(
        r#"rusticate-analyze-modules-mir - Analyze Rust stdlib usage from MIR files

USAGE:
    rusticate-analyze-modules-mir <PATH> [-j <N>]

OPTIONS:
    <PATH>                      Path to directory containing MIR files (from rusticate-mirify)
    -j, --jobs <N>              Number of parallel threads (default: 4)
    -h, --help                  Print this help message

DESCRIPTION:
    Analyzes MIR (Mid-level Intermediate Representation) files to understand stdlib usage.
    MIR provides fully-qualified paths for all function calls, method invocations, and types.
    
    From MIR we extract:
    - Direct method calls: Vec::new(), Result::unwrap()
    - Trait method calls: <Vec as IntoIterator>::into_iter
    - Type usage: std::result::Result<T, E>
...
EXAMPLES:
    # Analyze MIR files from RustCodebases
    rusticate-analyze-modules-mir ~/projects/RustCodebases

    # With more parallelism
    rusticate-analyze-modules-mir ~/projects/RustCodebases -j 8

PREREQUISITES:
    Run rusticate-mirify first to generate MIR files:
    rusticate-mirify ~/projects/RustCodebases
"#
    );
}
...
fn analyze_mir_multi_project(path: &Path, log_file: &mut fs::File) -> Result<()> {
    ...
    // Use word boundary \b to avoid matching std:: inside identifiers like "if_std::foo"
    let stdlib_path_re = Regex::new(r"\b(?:std|core|alloc)::[a-zA-Z_][a-zA-Z0-9_:]*").unwrap();
    ...
    let results: Vec<_> = projects_with_mir.par_iter().map(|project_path| {
        ...
        for mir_file in &mir_files {
            ...
            for cap in stdlib_path_re.find_iter(&filtered_content) {
                ...
                // Skip invalid symbols (C++ stdlib contamination)
                if !is_valid_stdlib_symbol(call) {
                    continue;
                }
                ...
                // Skip malformed entries (starting with ::)
            ...
            for cap in trait_impl_re.find_iter(&filtered_content) {
                let call = cap.as_str();
                let normalized = strip_generics(call);
                // Note: Don't insert `normalized` directly here - strip_generics on 
                // "<T as Trait>::method" produces "::method" which is invalid.
                // The qualified method is inserted below when we identify the type.
            ...
            for cap in stdlib_type_trait_impl_re.captures_iter(&filtered_content) {
            ...
            for cap in type_method_qualified_re.find_iter(&filtered_content) {
                let call = cap.as_str();
                let normalized = strip_generics(call);
            ...
            for cap in type_method_unqualified_re.captures_iter(&filtered_content) {
                if let (Some(type_match), Some(method_match)) = (cap.get(1), cap.get(2)) {
                    let type_name = type_match.as_str();
                    let method_name = method_match.as_str();
            ...
            for cap in type_annotation_re.captures_iter(&filtered_content) {
            ...
            for cap in type_constructor_re.captures_iter(&filtered_content) {
            ...
            for cap in trait_impl_pattern_re.captures_iter(&filtered_content) {
    ...
    println!("  7.   GREEDY TOUCH COVER: MODULES");
    println!("  8.   GREEDY FULL SUPPORT: MODULES");
    println!("  9.   GREEDY TOUCH COVER: DATA TYPES");
    println!(" 10.   GREEDY FULL SUPPORT: DATA TYPES");
    println!(" 11.   GREEDY TOUCH COVER: TRAITS");
    println!(" 12.   GREEDY FULL SUPPORT: TRAITS");
    println!(" 13.   GREEDY TOUCH COVER: METHODS/FUNCTIONS");
    println!(" 14.   GREEDY FULL SUPPORT: METHODS/FUNCTIONS");
    println!(" 15.   GREEDY COVER: METHODS PER TYPE");
    println!(" 16.   TRAIT IMPLEMENTATIONS AND GREEDY METHOD COVER");
    println!(" 17.   SUMMARY");
    println!();
    ...
    log!("  7.   GREEDY TOUCH COVER: MODULES");
    log!("  8.   GREEDY FULL SUPPORT: MODULES");
    log!("  9.   GREEDY TOUCH COVER: DATA TYPES");
    log!(" 10.   GREEDY FULL SUPPORT: DATA TYPES");
    log!(" 11.   GREEDY TOUCH COVER: TRAITS");
    log!(" 12.   GREEDY FULL SUPPORT: TRAITS");
    log!(" 13.   GREEDY TOUCH COVER: METHODS/FUNCTIONS");
    log!(" 14.   GREEDY FULL SUPPORT: METHODS/FUNCTIONS");
    log!(" 15.   GREEDY COVER: METHODS PER TYPE");
    log!(" 16.   TRAIT IMPLEMENTATIONS AND GREEDY METHOD COVER");
    log!(" 17.   SUMMARY");
    ...
    println!("7. GREEDY TOUCH COVER: MODULES");
    println!();
    println!("   'Touch' coverage: a crate is 'touched' if AT LEAST ONE module it uses is verified.");
    println!("   Finds minimum modules so that N%% of crates use at least one verified module.");
    println!("   Example: If we verify std::result, we 'touch' all crates that use Result.");
    println!("   This does NOT mean those crates are fully supported - they may use other modules.");
    println!();
    println!("8. GREEDY FULL SUPPORT: MODULES");
    println!();
    println!("   'Full support' coverage: a crate is 'fully supported' when ALL modules it uses");
    println!("   are verified. Finds minimum modules to fully support N%% of crates.");
    println!("   Example: A crate using std::result + std::io needs BOTH verified for full support.");
    println!("   This is the true verification cost - how much work to fully cover real codebases.");
    println!();
    println!("9. GREEDY TOUCH COVER: DATA TYPES");
    println!();
    println!("   'Touch' coverage for types: minimum types so N%% of crates use at least one.");
    println!("   Useful for understanding which types have the widest reach.");
    println!();
    println!("10. GREEDY FULL SUPPORT: DATA TYPES");
    println!();
    println!("   'Full support' coverage for types: minimum types to fully support N%% of crates.");
    println!("   A crate using Option + Vec + HashMap needs ALL THREE verified for full support.");
    println!();
    println!("11. GREEDY TOUCH COVER: TRAITS");
    println!();
    println!("   'Touch' coverage for traits: minimum traits so N%% of crates use at least one.");
    println!("   Shows which trait specs have the widest reach across codebases.");
    println!();
    println!("12. GREEDY FULL SUPPORT: TRAITS");
    println!();
    println!("   'Full support' coverage for traits: minimum traits to fully support N%% of crates.");
    println!("   A crate calling Iterator + Clone + Debug methods needs ALL verified for support.");
    println!();
    println!("13. GREEDY TOUCH COVER: METHODS/FUNCTIONS");
    println!();
    println!("   'Touch' coverage for methods: minimum methods so N%% of crates call at least one.");
    println!("   Key insight: ~9 methods touch 90%% of crates (Result::unwrap alone touches 57%%).");
    println!();
    println!("14. GREEDY FULL SUPPORT: METHODS/FUNCTIONS");
    println!();
    println!("   'Full support' coverage for methods: minimum methods to fully support N%% of crates.");
    println!("   This is the true method-level verification cost. A crate calling 50 methods needs");
    println!("   all 50 verified for full support - much harder than just 'touching' crates.");
    println!();
    println!("15. GREEDY COVER: METHODS PER TYPE");
    println!();
    println!("   For each major type (Vec, Option, Result, String, etc.), runs greedy touch cover");
    println!("   on its methods. Shows minimum methods needed per type for 70-100%% coverage.");
    println!("   Critical for prioritizing which Vec::* or Option::* methods to verify first.");
    println!();
    println!("16. TRAIT IMPLEMENTATIONS AND GREEDY METHOD COVER");
    println!();
    ...
    log!("\n7. GREEDY TOUCH COVER: MODULES");
    log!("");
    log!("   'Touch' coverage: a crate is 'touched' if AT LEAST ONE module it uses is verified.");
    log!("   Finds minimum modules so that N%% of crates use at least one verified module.");
    log!("   Example: If we verify std::result, we 'touch' all crates that use Result.");
    log!("   This does NOT mean those crates are fully supported - they may use other modules.");
    log!("\n8. GREEDY FULL SUPPORT: MODULES");
    log!("");
    log!("   'Full support' coverage: a crate is 'fully supported' when ALL modules it uses");
    log!("   are verified. Finds minimum modules to fully support N%% of crates.");
    log!("   Example: A crate using std::result + std::io needs BOTH verified for full support.");
    log!("   This is the true verification cost - how much work to fully cover real codebases.");
    log!("\n9. GREEDY TOUCH COVER: DATA TYPES");
    log!("");
    log!("   'Touch' coverage for types: minimum types so N%% of crates use at least one.");
    log!("   Useful for understanding which types have the widest reach.");
    log!("\n10. GREEDY FULL SUPPORT: DATA TYPES");
    log!("");
    log!("   'Full support' coverage for types: minimum types to fully support N%% of crates.");
    log!("   A crate using Option + Vec + HashMap needs ALL THREE verified for full support.");
    log!("\n11. GREEDY TOUCH COVER: TRAITS");
    log!("");
    log!("   'Touch' coverage for traits: minimum traits so N%% of crates use at least one.");
    log!("   Shows which trait specs have the widest reach across codebases.");
    log!("\n12. GREEDY FULL SUPPORT: TRAITS");
    log!("");
    log!("   'Full support' coverage for traits: minimum traits to fully support N%% of crates.");
    log!("   A crate calling Iterator + Clone + Debug methods needs ALL verified for support.");
    log!("\n13. GREEDY TOUCH COVER: METHODS/FUNCTIONS");
    log!("");
    log!("   'Touch' coverage for methods: minimum methods so N%% of crates call at least one.");
    log!("   Key insight: ~9 methods touch 90%% of crates (Result::unwrap alone touches 57%%).");
    log!("\n14. GREEDY FULL SUPPORT: METHODS/FUNCTIONS");
    log!("");
    log!("   'Full support' coverage for methods: minimum methods to fully support N%% of crates.");
    log!("   This is the true method-level verification cost. A crate calling 50 methods needs");
    log!("   all 50 verified for full support - much harder than just 'touching' crates.");
    log!("\n15. GREEDY COVER: METHODS PER TYPE");
    log!("");
    ...
    log!("\n16. TRAIT IMPLEMENTATIONS AND GREEDY METHOD COVER");
    log!("");
    ...
    // 
    // OPTIMIZED: Uses incremental tracking instead of recomputing from scratch.
    // For each crate, track how many of its items are NOT yet verified.
    // When that count reaches 0, the crate is fully supported.
    fn greedy_cover_full_support(
        ...
        // Build item -> crates mapping (inverse of crate_to_items)
        ...
        // For each crate, track how many items remain unverified
        let mut remaining_count: BTreeMap<String, usize> = crate_to_items.iter()
            .map(|(name, items)| (name.clone(), items.len()))
            .collect();
        
        // Crates with 0 items are already "fully supported"
        ...
        while current_supported < target_count {
            ...
            // An item helps if verifying it causes some crate's remaining_count to hit 0
            ...
            for (item, crates) in &item_to_crates {
                ...
                // Count how many crates would become fully supported
            ...
            if let Some(item) = best_item {
                // Verify this item: decrement remaining_count for all crates using it
                    ...
                    for crate_name in crates {
                        if let Some(count) = remaining_count.get_mut(crate_name) {
                            if *count > 0 {
                                *count -= 1;
                                if *count == 0 {
                                    // This crate just became fully supported
                                }
                            }
                        }
                    }
                ...
                // No single item improves support - pick the item used by most crates
                // (prioritize items that reduce the most remaining counts)
                let mut best_coverage = 0;
                for (item, crates) in &item_to_crates {
                    if verified_items.contains(item) { continue; }
                    let coverage = crates.iter()
                        .filter(|c| remaining_count.get(*c).copied().unwrap_or(0) > 0)
                        .count();
    ...
    // =========================================================================
    // === GREEDY TOUCH COVER: MODULES ===
    // Description: Find minimum set of stdlib modules to TOUCH N% of crates.
    // A crate is "touched" when at least ONE module it uses is verified.
    // =========================================================================
    ...
    // Build inverse mapping: crate -> modules it uses (for full support)
    let mut crate_to_modules: BTreeMap<String, HashSet<String>> = BTreeMap::new();
    for (module, crates) in &module_total_crates {
        for crate_name in crates {
            crate_to_modules.entry(crate_name.clone()).or_default().insert(module.clone());
        }
    }
    
    println!("\n=== 7. GREEDY TOUCH COVER: MODULES ===");
    println!("'Touch' coverage: A crate is touched when AT LEAST ONE module it uses is verified.");
    println!("Minimum modules to touch (70/80/90/95/99/100)% of {} crates.", module_covered_count);
    println!("Excludes {} crates with no stdlib usage, {} more with no module usage.", uncovered_count, module_uncovered_count);
    println!("{}", "=".repeat(80));
    log!("\n=== 7. GREEDY TOUCH COVER: MODULES ===");
    log!("'Touch' coverage: A crate is touched when AT LEAST ONE module it uses is verified.");
    log!("Minimum modules to touch (70/80/90/95/99/100)% of {} crates.", module_covered_count);
    ...
    for &target in &targets {
        ...
        println!("  => {} modules touch {:.4}%", result.len(), achieved);
        log!("  => {} modules touch {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY FULL SUPPORT: MODULES ===
    // Description: Find minimum set of stdlib modules to FULLY SUPPORT N% of crates.
    // A crate is "fully supported" when ALL modules it uses are verified.
    // =========================================================================
    
    println!("\n\n=== 8. GREEDY FULL SUPPORT: MODULES ===");
    println!("'Full support' coverage: A crate is fully supported when ALL stdlib modules it uses are verified.");
    println!("Minimum modules to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_modules.len());
    println!("{}", "=".repeat(80));
    log!("\n\n=== 8. GREEDY FULL SUPPORT: MODULES ===");
    log!("'Full support' coverage: A crate is fully supported when ALL stdlib modules it uses are verified.");
    log!("Minimum modules to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_modules.len());
    
    for &target in &targets {
        let result = greedy_cover_full_support(&crate_to_modules, target);
        let achieved = result.last().map(|(_, _, p)| *p).unwrap_or(0.0);
        
        println!("\n--- Target: {:.0}% ({} crates) ---", target, 
            (crate_to_modules.len() as f64 * target / 100.0).ceil() as usize);
        log!("\n--- Target: {:.0}% ({} crates) ---", target,
            (crate_to_modules.len() as f64 * target / 100.0).ceil() as usize);
        
        // Show first 20 for stdout, all in log
        for (i, (name, new_cov, cum_pct)) in result.iter().enumerate() {
            let display = if name.len() > 40 { format!("{}...", &name[..37]) } else { name.clone() };
            if i < 20 {
                println!("  {:3}. {:40} +{:>5} ({:>8.4}%)", i + 1, display, new_cov, cum_pct);
            }
            log!("  {:3}. {:40} +{:>5} ({:>8.4}%)", i + 1, name, new_cov, cum_pct);
        }
        if result.len() > 20 {
        ...
        println!("  => {} modules fully support {:.4}%", result.len(), achieved);
        log!("  => {} modules fully support {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY TOUCH COVER: DATA TYPES ===
    // Description: Find minimum set of stdlib types to TOUCH N% of crates.
    // =========================================================================
    ...
    // Build inverse mapping: crate -> types it uses (for full support)
    let mut crate_to_types: BTreeMap<String, HashSet<String>> = BTreeMap::new();
    for (type_name, crates) in &type_total_crates {
        for crate_name in crates {
            crate_to_types.entry(crate_name.clone()).or_default().insert(type_name.clone());
        }
    }
    
    println!("\n\n=== 9. GREEDY TOUCH COVER: DATA TYPES ===");
    println!("'Touch' coverage: A crate is touched when AT LEAST ONE type it uses is verified.");
    println!("Minimum types to touch (70/80/90/95/99/100)% of {} crates.", type_covered_count);
    println!("Excludes {} crates with no stdlib usage, {} more with no type usage.", uncovered_count, type_uncovered_count);
    println!("{}", "=".repeat(80));
    log!("\n\n=== 9. GREEDY TOUCH COVER: DATA TYPES ===");
    log!("'Touch' coverage: A crate is touched when AT LEAST ONE type it uses is verified.");
    log!("Minimum types to touch (70/80/90/95/99/100)% of {} crates.", type_covered_count);
    ...
    for &target in &targets {
        ...
        println!("  => {} types touch {:.4}%", result.len(), achieved);
        log!("  => {} types touch {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY FULL SUPPORT: DATA TYPES ===
    // Description: Find minimum set of stdlib types to FULLY SUPPORT N% of crates.
    // =========================================================================
    
    println!("\n\n=== 10. GREEDY FULL SUPPORT: DATA TYPES ===");
    println!("'Full support' coverage: A crate is fully supported when ALL stdlib types it uses are verified.");
    println!("Minimum types to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_types.len());
    println!("{}", "=".repeat(80));
    log!("\n\n=== 10. GREEDY FULL SUPPORT: DATA TYPES ===");
    log!("'Full support' coverage: A crate is fully supported when ALL stdlib types it uses are verified.");
    log!("Minimum types to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_types.len());
    
    for &target in &targets {
        let result = greedy_cover_full_support(&crate_to_types, target);
        let achieved = result.last().map(|(_, _, p)| *p).unwrap_or(0.0);
        
        println!("\n--- Target: {:.0}% ({} crates) ---", target, 
            (crate_to_types.len() as f64 * target / 100.0).ceil() as usize);
        log!("\n--- Target: {:.0}% ({} crates) ---", target,
            (crate_to_types.len() as f64 * target / 100.0).ceil() as usize);
        ...
        println!("  => {} types fully support {:.4}%", result.len(), achieved);
        log!("  => {} types fully support {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY TOUCH COVER: TRAITS ===
    // Description: Find minimum set of stdlib traits to TOUCH N% of crates.
    // =========================================================================
    ...
    // Build inverse mapping: crate -> traits it uses (for full support)
    let mut crate_to_traits: BTreeMap<String, HashSet<String>> = BTreeMap::new();
    for (trait_name, crates) in &trait_total_crates {
        for crate_name in crates {
            crate_to_traits.entry(crate_name.clone()).or_default().insert(trait_name.clone());
        }
    }
    
    println!("\n\n=== 11. GREEDY TOUCH COVER: TRAITS ===");
    println!("'Touch' coverage: A crate is touched when AT LEAST ONE trait it uses is verified.");
    println!("Minimum traits to touch (70/80/90/95/99/100)% of {} crates.", trait_covered_count);
    ...
    println!("{}", "=".repeat(80));
    log!("\n\n=== 11. GREEDY TOUCH COVER: TRAITS ===");
    log!("'Touch' coverage: A crate is touched when AT LEAST ONE trait it uses is verified.");
    log!("Minimum traits to touch (70/80/90/95/99/100)% of {} crates.", trait_covered_count);
    ...
    for &target in &targets {
        ...
        println!("  => {} traits touch {:.4}%", result.len(), achieved);
        log!("  => {} traits touch {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY FULL SUPPORT: TRAITS ===
    // Description: Find minimum set of stdlib traits to FULLY SUPPORT N% of crates.
    // =========================================================================
    
    println!("\n\n=== 12. GREEDY FULL SUPPORT: TRAITS ===");
    println!("'Full support' coverage: A crate is fully supported when ALL stdlib traits it uses are verified.");
    println!("Minimum traits to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_traits.len());
    println!("{}", "=".repeat(80));
    log!("\n\n=== 12. GREEDY FULL SUPPORT: TRAITS ===");
    log!("'Full support' coverage: A crate is fully supported when ALL stdlib traits it uses are verified.");
    log!("Minimum traits to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_traits.len());
    
    for &target in &targets {
        let result = greedy_cover_full_support(&crate_to_traits, target);
        let achieved = result.last().map(|(_, _, p)| *p).unwrap_or(0.0);
        
        println!("\n--- Target: {:.0}% ({} crates) ---", target, 
            (crate_to_traits.len() as f64 * target / 100.0).ceil() as usize);
        log!("\n--- Target: {:.0}% ({} crates) ---", target,
            (crate_to_traits.len() as f64 * target / 100.0).ceil() as usize);
        ...
        println!("  => {} traits fully support {:.4}%", result.len(), achieved);
        log!("  => {} traits fully support {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY TOUCH COVER: METHODS/FUNCTIONS ===
    // Description: Find minimum set of stdlib methods to TOUCH N% of crates.
    // =========================================================================
    ...
    // Build inverse mapping: crate -> methods it uses (for full support)
    let mut crate_to_methods: BTreeMap<String, HashSet<String>> = BTreeMap::new();
    for (method_name, crates) in &filtered_methods {
        for crate_name in crates {
            crate_to_methods.entry(crate_name.clone()).or_default().insert(method_name.clone());
        }
    }
    
    println!("\n\n=== 13. GREEDY TOUCH COVER: METHODS/FUNCTIONS ===");
    println!("'Touch' coverage: A crate is touched when AT LEAST ONE method it calls is verified.");
    println!("Minimum methods to touch (70/80/90/95/99/100)% of {} crates.", method_covered_count);
    ...
    println!("{}", "=".repeat(80));
    log!("\n\n=== 13. GREEDY TOUCH COVER: METHODS/FUNCTIONS ===");
    log!("'Touch' coverage: A crate is touched when AT LEAST ONE method it calls is verified.");
    log!("Minimum methods to touch (70/80/90/95/99/100)% of {} crates.", method_covered_count);
    ...
    for &target in &targets {
        ...
        println!("  => {} methods touch {:.4}%", result.len(), achieved);
        log!("  => {} methods touch {:.4}%", result.len(), achieved);
    }
    
    // =========================================================================
    // === GREEDY FULL SUPPORT: METHODS/FUNCTIONS ===
    // Description: Find minimum set of stdlib methods to FULLY SUPPORT N% of crates.
    // =========================================================================
    
    println!("\n\n=== 14. GREEDY FULL SUPPORT: METHODS/FUNCTIONS ===");
    println!("'Full support' coverage: A crate is fully supported when ALL methods it calls are verified.");
    println!("Minimum methods to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_methods.len());
    println!("NOTE: This is much harder than touch coverage - most crates call many methods.");
    println!("{}", "=".repeat(80));
    log!("\n\n=== 14. GREEDY FULL SUPPORT: METHODS/FUNCTIONS ===");
    log!("'Full support' coverage: A crate is fully supported when ALL methods it calls are verified.");
    log!("Minimum methods to fully support (70/80/90/95/99/100)% of {} crates.", crate_to_methods.len());
    log!("NOTE: This is much harder than touch coverage - most crates call many methods.");
    
    // Show distribution of methods per crate
    let mut method_counts: Vec<usize> = crate_to_methods.values().map(|s| s.len()).collect();
    method_counts.sort();
    let median_methods = if method_counts.is_empty() { 0 } else { method_counts[method_counts.len() / 2] };
    let max_methods = method_counts.iter().max().copied().unwrap_or(0);
    let min_methods = method_counts.iter().min().copied().unwrap_or(0);
    println!("\nMethods per crate: min={}, median={}, max={}", min_methods, median_methods, max_methods);
    log!("\nMethods per crate: min={}, median={}, max={}", min_methods, median_methods, max_methods);
    
    for &target in &targets {
        let result = greedy_cover_full_support(&crate_to_methods, target);
        let achieved = result.last().map(|(_, _, p)| *p).unwrap_or(0.0);
        
        println!("\n--- Target: {:.0}% ({} crates) ---", target, 
            (crate_to_methods.len() as f64 * target / 100.0).ceil() as usize);
        log!("\n--- Target: {:.0}% ({} crates) ---", target,
            (crate_to_methods.len() as f64 * target / 100.0).ceil() as usize);
        
        // Show first 30 for stdout (these lists can get long), all in log
        for (i, (name, new_cov, cum_pct)) in result.iter().enumerate() {
            let display = if name.len() > 50 { format!("{}...", &name[..47]) } else { name.clone() };
            if i < 30 {
                println!("  {:3}. {:50} +{:>5} ({:>8.4}%)", i + 1, display, new_cov, cum_pct);
        ...
        if result.len() > 30 {
            println!("  ... ({} more in log)", result.len() - 30);
        }
        println!("  => {} methods fully support {:.4}%", result.len(), achieved);
        log!("  => {} methods fully support {:.4}%", result.len(), achieved);
    }
    ...
    println!("Description: For each data type, minimum methods to touch 70/80/90/95/99/100% of its users");
    println!("{}", "=".repeat(80));
    log!("\n\n=== 15. GREEDY COVER: METHODS PER TYPE ===");
    log!("This helps us answer: For each type, which methods matter most?");
    log!("Description: For each data type, minimum methods to touch 70/80/90/95/99/100% of its users");
...
fn main() -> Result<()> {
    ...
    // Run MIR analysis
    analyze_mir_multi_project(&args.mir_path, &mut log_file)?;
