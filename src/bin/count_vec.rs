...
fn main() -> Result<()> {
    ...
    if !other_dirs.is_empty() {
        ...
        for file in all_files {
            match count_vec_in_file(&file) {
                Ok(count) => {
                    if let Ok(rel_path) = file.strip_prefix(&base_dir) {
                        println!("{}: {} Vec usages", rel_path.display(), format_number(count));
                    } else {
                        println!("{}: {} Vec usages", file.display(), format_number(count));
                    }
                }
                Err(e) => {
                    eprintln!("Warning: Failed to parse {}: {}", file.display(), e);
    ...
    for (section, files) in &section_counts {
        ...
        for (file, count) in files {
            println!("  {}: {} Vec usages", file, format_number(*count));
        }
        println!();
    }
    ...
    println!("Summary: {} Vec usages", summary_parts.join(", "));
