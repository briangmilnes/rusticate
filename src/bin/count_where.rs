...
fn main() -> Result<()> {
    ...
    for (name, dirs) in &categories {
        ...
        for file in files {
            match count_where_in_file(&file) {
                Ok(count) => {
    ...
    if !other_dirs.is_empty() {
        ...
        for file in all_files {
            match count_where_in_file(&file) {
                Ok(count) => {
                    if let Ok(rel_path) = file.strip_prefix(&base_dir) {
                        println!("{}: {} where clauses", rel_path.display(), format_number(count));
                    } else {
                        println!("{}: {} where clauses", file.display(), format_number(count));
                    }
                }
                Err(e) => {
                    eprintln!("Warning: Failed to parse {}: {}", file.display(), e);
    ...
    for (section, files) in &section_counts {
        ...
        for (file, count) in files {
            println!("  {}: {} where clauses", file, format_number(*count));
        }
        println!();
    }
    ...
    println!("Summary: {} where clauses", summary_parts.join(", "));
