...
fn main() -> Result<()> {
    ...
    if args.len() < 2 || args.contains(&"-h".to_string()) || args.contains(&"--help".to_string()) {
        log!("Usage: compile [OPTIONS]");
        log!("");
        log!("Options:");
        log!("  -c, --codebase             Compile src/, tests/, benches/ (default)");
        log!("  -d, --dir DIR              Compile specific directory");
        log!("  -f, --file FILE            Compile checking single file");
        log!("  -m, --module NAME          Compile module: src/ChapNN/ModuleName.rs, tests/ChapNN/TestModuleName.rs, benches/ChapNN/BenchModuleName.rs");
        log!("  -h, --help                 Show this help message");
        log!("");
        log!("Examples:");
        log!("  compile                    # Compile entire codebase");
        log!("  compile -c                 # Same as above");
        log!("  compile -m SetStEph        # Compile src, test, and bench for SetStEph");
        log!("  compile -f src/lib.rs      # Check single file compiles");
        log!("  compile -d src/Chap05      # Compile directory");
        return Ok(());
    }
