...
Strategy:
1. Add logging setup after fn main()
2. Replace println! with log! ONLY after the log! macro definition
"""
...
'''
    ...
    macro_end_marker = "    }\n\n"
    content = content[:match.end()] + logging_code + content[match.end():]
    
    # Now find where the macro ends (after the inserted code)
    # Look for the line "    }" followed by blank line after the log macro
    macro_pattern = r'(macro_rules! log \{.*?\n    \}\s*\n\s*\n)'
    macro_match = re.search(macro_pattern, content, re.DOTALL)
    
    if macro_match:
        # Split content at macro end
        before_macro = content[:macro_match.end()]
        after_macro = content[macro_match.end():]
        
        # Replace println! with log! only in the part AFTER the macro
        after_macro = re.sub(r'\bprintln!\s*\(', 'log!(', after_macro)
        
        content = before_macro + after_macro
