...
Usage:
  ...
  fix_clone_display_to_stt.py <file.rs> --log_file <path>  # Custom log path
"""
...
def fix_clone_display_to_stt(file_path, dry_run=False, tee=None):
...
def main():
    parser = argparse.ArgumentParser(description='Fix Clone + Display bounds to use StT')
    parser.add_argument('file', type=Path, help='File to fix')
    parser.add_argument('--dry-run', action='store_true', help='Show changes without applying')
    parser.add_argument('--log_file', 
                       default='analyses/code_review/fix_clone_display_to_stt.txt',
                       help='Path to log file (default: analyses/code_review/fix_clone_display_to_stt.txt)')
    ...
    tee.print(f"{'='*60}")
    tee.print(f"fix_clone_display_to_stt.py: {file_path}")
    tee.print(f"{'='*60}")
    
    success = fix_clone_display_to_stt(file_path, dry_run=dry_run, tee=tee)
